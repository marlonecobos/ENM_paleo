...
mammothOccurrencesClean[,c("lng", "lat", "early_age", "late_age")] <- as.numeric(unlist(mammothOccurrencesClean[,c("lng", "lat", "early_age", "late_age")]))

# Quick plot to see what's there
plot(mammothOccurrencesClean[,c("lng","lat")], pch = 20, col = "red", main = "All Mammoth Points", xlab = "Longitude", ylab = "Latitude")
plot(gshhs, add = T)
...
# Assign time slices to occcurences based on date ranges from paleoclim.org
...
legend("topleft",title="Date (My)",legend=relevantTimeSlices$meanAge[sort(unique(mammothOccurrencesClean$timeIndex))],col =rbPal(ncolors),pch=20)
plot(gshhs, add = T)
